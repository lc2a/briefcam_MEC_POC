...
class BriefcamParser():
    ...
    def import_environment_variables(self):
        ...
        self.__write_log_to_redis_and_logging_framework("case_name={}"
                                                        .format(self.case_name), True)
        self.__write_log_to_redis_and_logging_framework("case_url={}"
                                                        .format(self.case_url), True)
        self.__write_log_to_redis_and_logging_framework("username={}"
                                                        .format(self.username))
        self.__write_log_to_redis_and_logging_framework("browser_loc={}"
                                                        .format(self.browser_loc), True)
        self.__write_log_to_redis_and_logging_framework("browser_name={}"
                                                        .format(self.browser_name), True)
        self.__write_log_to_redis_and_logging_framework("image_directory={}"
                                                        .format(self.image_directory), True)
        self.__write_log_to_redis_and_logging_framework("max_retry_attempts={}"
                                                        .format(self.max_retry_attempts), True)
        self.__write_log_to_redis_and_logging_framework("sleep_time={}"
                                                        .format(self.sleep_time), True)
        self.__write_log_to_redis_and_logging_framework("time_between_input_character={}"
                                                        .format(self.time_between_input_character), True)
        self.__write_log_to_redis_and_logging_framework("time_for_browser_to_open={}"
                                                        .format(self.time_for_browser_to_open), True)
        self.__write_log_to_redis_and_logging_framework("total_job_done_count_redis_name={}"
                                                        .format(self.total_job_done_count_redis_name), True)
        self.__write_log_to_redis_and_logging_framework("redis_log_keyname={}"
                                                        .format(self.redis_log_keyname), True)
        self.__write_log_to_redis_and_logging_framework("video_file_path={}".format(self.video_file_path))

    def __filename_formatted(self, filename):
    ...
    def __proceed_with_execution(self):
        ...
        text = pyautogui.confirm(text='Shall I proceed in creating a case?',
                                 title='Question',
                                 buttons=['OK', 'Cancel'])
        if text == 'Cancel':
            return False
    ...
    def _left_click_this_coordinate(self, coordinates):
        if coordinates is None:
            return
        event_log = "Trying to left click this coordinate[{},{}] "\
            .format(coordinates[0],
                    coordinates[1])
    ...
    def __check_for_background_process(self):
        ...
        if self.browser_name in result:
            self.__write_log_to_redis_and_logging_framework("process {} is "
                                                          "running.".format(self.browser_name))
            return True
        else:
            self.__write_log_to_redis_and_logging_framework("Cannot find process {} "
                                                          "running.".format(self.browser_name))
            return False
    ...
    def __leftclick_add_video_button(self):
        ...
        for index in range(4):
            return_value = self.__left_click_this_image(
                self.__filename_formatted('add_video_to_case_button.png'), False)
            pyautogui.press('esc')
            if return_value:
                break
            time.sleep(self.time_between_input_character)
    ...
    def __add_video(self, file_name):
        ...
        self.__left_click_this_image(self.__filename_formatted('next2_button.png'))
        time.sleep(self.sleep_time)
    ...
    def prepare_browser(self, skip_login=False):
        if self.process is None:
            ...
            if self.process is None:
                self.__write_log_to_redis_and_logging_framework("Process is None")
